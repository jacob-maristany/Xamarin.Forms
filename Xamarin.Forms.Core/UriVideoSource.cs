...
namespace Xamarin.Forms
{
	...
	{
		internal const string CacheName = "VideoLoaderCache";

		public static readonly BindableProperty UriProperty = BindableProperty.Create("Uri", typeof(Uri), typeof(UriVideoSource), default(Uri),
			propertyChanged: (bindable, oldvalue, newvalue) => ((UriVideoSource)bindable).OnUriChanged(), validateValue: (bindable, value) => value == null || ((Uri)value).IsAbsoluteUri);
		...
		public async Task<Stream> GetStreamAsync(CancellationToken userToken = default(CancellationToken))
		{
			...
			{
				Log.Warning("Video Loading", $"Error getting stream for {Uri}: {ex}");
				throw;
			}
		...
		static string GetCacheKey(Uri uri) => Device.PlatformServices.GetMD5Hash(uri.AbsoluteUri);
		...
		async Task<Stream> GetStreamAsync(Uri uri, CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			if (CachingEnabled)
				return await GetStreamFromCacheAsync(uri, cancellationToken).ConfigureAwait(false);

			try
			{
				return await Device.GetStreamAsync(uri, cancellationToken).ConfigureAwait(false);
			}
			catch (Exception ex)
			{
				Log.Warning("Video Loading", $"Error getting stream for {Uri}: {ex}");
